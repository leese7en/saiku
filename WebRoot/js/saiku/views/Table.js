...
var Table = Backbone.View.extend( {
	...
	clicked_cell : function(event) {
		...
		var $target = ($(event.target).hasClass('row') || $(event.target).hasClass('col')) ? $(event.target)
				.find('div') : $(event.target);
		...
		$.contextMenu( {
			...
			build : function($trigger, e) {
				...
				var cube = query.get('connection') + "/" + query.get('catalog') + "/"
						+ ((schema == "" || schema == null) ? "null" : schema) + "/" + query.get('cube');
				...
				if (!dimensions) {
					Saiku.session.sessionworkspace.cube[cube].fetch( {
						async : false
					});
						...
						_.each(dimension.hierarchies, function(hierarchy) {
							if (hierarchy.uniqueName == h) {
								_.each(hierarchy.levels, function(level) {
									...
									items["keep-" + level.name].payload = keep_payload + ","
				...
				items["getchildren"] = {
					payload : children_payload
				};
				if (items.hasOwnProperty("remove-" + l_name) && items.hasOwnProperty("include-" + l_name)) {
					items["showall"] = {
						payload : items["remove-" + l_name].payload + ", " + items["include-" + l_name].payload
					};
				}
				...
				return {
					callback : function(key, options) {
						...
						if (key.indexOf("filterlevel") >= 0) {
							var key = encodeURIComponent(d) + "/hierarchy/" + encodeURIComponent(h) + "/"
						...
						if (children) {
							self.workspace.query.set( {
								'formatter' : 'flat'
							});
						}
						self.workspace.query.action.put(url, {
							success : self.workspace.sync_query,
							dataType : "text",
							data : children ? {
