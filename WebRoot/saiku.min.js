var Settings = {
	...
	LOCALSTORAGE_EXPIRATION : 10 * 60 * 60 * 1000,
	UPGRADE : true
};
...
function replaceTag(a) {
	return tagsToReplace[a] || a
}
function safe_tags_replace(a) {
	return a.replace(/[&<>]/g, replaceTag)
}
if ($.blockUI) {
...
if (window.location.hostname
		&& (window.location.hostname == "dev.analytical-labs.com" || window.location.hostname == "demo.analytical-labs.com")) {
...
var SaikuRenderer = function(b, a) {
	this._options = _.extend(SaikuRendererOptions, a);
...
SaikuRenderer.prototype.render = function(c, a) {
	var b = null;
	if (typeof Backbone !== "undefined") {
		this.trigger("render:start", this)
	}
	if (!this.hasProcessedData()) {
		this.processData(c, a)
	}
	b = this._render(c, a);
	if (typeof Backbone !== "undefined") {
...
SaikuRenderer.prototype.processData = function(b, a) {
	if (typeof Backbone !== "undefined") {
...
SaikuRenderer.prototype._processData = function(b, a) {
};
var SaikuTableRenderer = _.extend(SaikuRenderer, {
	key : "table"
});
SaikuTableRenderer.prototype._render = function(d, b) {
	var a = this;
	if (d) {
		this._data = d
	}
	...
	if (this._options.htmlObject) {
		if (a._options.hasOwnProperty("batch")) {
			$(a._options.htmlObject).parent().parent().unbind("scroll")
		}
		_.defer(function(f) {
			if (a._options.hasOwnProperty("batch") && !a._options.hasOwnProperty("batchSize")) {
				a._options.batchSize = 1000
			}
			var e = a.internalRender(a._data, a._options);
			$(a._options.htmlObject).html(e);
			_.defer(function(l) {
				if (a._options.hasOwnProperty("batch") && a._options.hasBatchResult) {
					var j = 0;
					var m = false;
					var k = a._options.hasOwnProperty("batchIntervalSize") ? a._options.batchIntervalSize : 20;
					var n = a._options.hasOwnProperty("batchIntervalTime") ? a._options.batchIntervalTime : 20;
					var g = a._options.batchResult.length;
					var i = function() {
						if (!m && g > 0 && j < g) {
							m = true;
							var p = "";
							var o = j;
							for ( var q = 0; j < g && q < k; q++, j++) {
								p += a._options.batchResult[j]
							}
							if (j > o) {
								$(a._options.htmlObject).append($(p))
							}
							m = false
						}
						if (j >= g) {
					...
					var h = _.debounce(i, n);
					$(a._options.htmlObject).parent().parent().scroll(function() {
						h()
					})
				}
			});
		...
		var c = this.internalRender(this._data, a._options);
		return c
	}
};
SaikuTableRenderer.prototype.clear = function(c, b) {
	var a = this;
	if (this._options.htmlObject && this._options.hasOwnProperty("batch")) {
		$(a._options.htmlObject).parent().parent().unbind("scroll")
	}
};
SaikuTableRenderer.prototype._processData = function(b, a) {
	this._hasProcessed = true
};
function genTotalDataCells(g, h, a, c, j) {
	var e = "";
	var j = j[ROWS];
	for ( var f = a.length - 1; f >= 0; f--) {
		if (g == a[f]) {
			...
			for ( var d = 0; d < b.cells.length; d++) {
				e += '<td class="data total">' + b.cells[d][h].value + "</td>"
			}
			c[f]++;
			if (c[f] < j[f].length) {
				a[f] += j[f][c[f]].width
...
function genTotalHeaderCells(h, a, b, d, k, j) {
	var f = "";
	for ( var g = a; g >= 0; g--) {
		if (h == b[g]) {
			var c = k[g][d[g]];
			...
			for ( var e = 0; e < c.cells.length; e++) {
				...
				if (a == k.length - 1) {
					...
					if (g == 0 && d[g] == 0) {
						...
						l += (j ? "<span class='i18n'>Grand Total</span>" : "Grand Total")
					}
				}
				f += '<th class="' + n + '">' + (j ? "<div>" + l + "</div>" : l) + "</th>"
			}
			d[g]++;
			if (d[g] < k[g].length) {
				b[g] += k[g][d[g]].width
...
function totalIntersectionCells(h, a, b, d, j) {
	var f = "";
	for ( var g = a; g >= 0; g--) {
		if (h == b[g]) {
			var c = j[g][d[g]];
...
function genTotalHeaderRowCells(p, b, c, q, s) {
	var d = q[COLUMNS];
	var e = b[COLUMNS];
	var o = c[COLUMNS];
	var a = d.length - 2;
	...
	for ( var n = a; n >= 0; n--) {
		if (p == e[n]) {
			for ( var f = 0; f < d[n][o[n]].cells.length; f++) {
				g += "<tr>";
				for ( var l = 0; l <= a; l++) {
					var u;
					var t = "&nbsp;";
					...
					if (l == a) {
						...
						if (n == 0 && o[n] == 0) {
							...
							t += (s ? "<span class='i18n'>Grand Total</span>" : "Grand Total")
						}
					}
					g += '<th class="' + u + '">' + (s ? "<div>" + t + "</div>" : t) + "</th>"
				}
				var c = {};
				var b = {};
				for ( var r = 0; r < q[ROWS].length; r++) {
					c[r] = 0;
					b[r] = q[ROWS][r][c[r]].width
				}
				for ( var h = 0; h < d[n][o[n]].cells[f].length; h++) {
					g += '<td class="data total">' + d[n][o[n]].cells[f][h].value + "</td>";
					g += totalIntersectionCells(h + 1, q[ROWS].length - 1, b, c, q[ROWS])
				}
				g += "</tr>"
			}
			o[n]++;
...
var ROWS = "ROWS";
var COLUMNS = "COLUMNS";
function nextParentsDiffer(b, c, a) {
	while (c-- > 0) {
		if (b[c][a].properties.uniquename != b[c][a + 1].properties.uniquename) {
			return true
		}
	}
	return false
}
function topParentsDiffer(b, c, a) {
	while (a-- > 0) {
		if (b[c][a].properties.uniquename != b[c - 1][a].properties.uniquename) {
			return true
		}
	}
	return false
}
SaikuTableRenderer.prototype.internalRender = function(j, D) {
	var p = "";
	var y = "";
	var ad = j.cellset;
	var x = ad ? ad : [];
	...
	var Z = true;
	if (D) {
		q = D.hasOwnProperty("batchSize") ? D.batchSize : null;
		Z = D.hasOwnProperty("wrapContent") ? D.wrapContent : true
	}
	var n = {};
	n[COLUMNS] = j.rowTotalsLists;
	n[ROWS] = j.colTotalsLists;
	var ac = {};
	var P = {};
	var C = [ ROWS, COLUMNS ];
	...
	if (n[COLUMNS]) {
		for ( var aa = 0; aa < n[COLUMNS].length; aa++) {
			P[COLUMNS][aa] = 0;
			ac[COLUMNS][aa] = n[COLUMNS][aa][P[COLUMNS][aa]].width
		}
	}
	for ( var r = 0, t = x.length; r < t; r++) {
		...
		var L = false;
		if (n[ROWS]) {
			for ( var aa = 0; aa < n[ROWS].length; aa++) {
				P[ROWS][aa] = 0;
				ac[ROWS][aa] = n[ROWS][aa][P[ROWS][aa]].width
			}
		}
		y = "<tr>";
		if (r === 0) {
		...
		for ( var h = 0, d = x[r].length; h < d; h++) {
			var W = h - j.leftOffset;
			var c = ad[r][h];
			if (c.type === "COLUMN_HEADER") {
				T = true
			}
			if (c.type === "COLUMN_HEADER" && c.value === "null" && (B == null || h < B)) {
				y += '<th class="all_null">&nbsp;</th>'
			} else {
				if (c.type === "COLUMN_HEADER") {
					if (B == null) {
						B = h
					...
					if (z) {
						if (c.value == "null") {
							y += '<th class="col_null">&nbsp;</th>'
						} else {
							if (n[ROWS]) {
								m = n[ROWS][r + 1][P[ROWS][r + 1]].span
							}
							y += '<th class="col" style="text-align: center;" colspan="' + m + '" title="' + c.value
									+ '">' + (Z ? '<div rel="' + r + ":" + h + '">' + c.value + "</div>" : c.value)
									+ "</th>"
						}
					} else {
						var ab = (h > 1 && r > 1 && !U && h > B) ? ad[r - 1][h + 1].value != ad[r - 1][h].value
								|| ad[r - 1][h + 1].properties.uniquename != ad[r - 1][h].properties.uniquename : false;
						var O = m > 999 ? true : false;
						if (c.value != g.value || nextParentsDiffer(ad, r, h) || U || ab || O) {
							if (c.value == "null") {
								y += '<th class="col_null" colspan="' + m + '">&nbsp;</th>'
							} else {
								if (n[ROWS]) {
									m = n[ROWS][r + 1][P[ROWS][r + 1]].span
								}
								y += '<th class="col" style="text-align: center;" colspan="' + (m == 0 ? 1 : m)
										+ '" title="' + c.value + '">'
										+ (Z ? '<div rel="' + r + ":" + h + '">' + c.value + "</div>" : c.value)
										+ "</th>"
							}
					...
					if (n[ROWS]) {
						y += genTotalHeaderCells(h - j.leftOffset + 1, r + 1, ac[ROWS], P[ROWS], n[ROWS], Z)
					}
				} else {
					if (c.type === "ROW_HEADER" && c.value === "null") {
						y += '<th class="row_null">&nbsp;</th>'
					} else {
						if (c.type === "ROW_HEADER") {
							if (a == h) {
								U = true
							} else {
								g = ad[r][h + 1]
							...
							var M = ad[r - 1];
							var s = !L && !U && (h == 0 || !topParentsDiffer(ad, r, h)) && c.value === M[h].value;
							L = !s;
							var S = (s ? "<div>&nbsp;</div>" : '<div rel="' + r + ":" + h + '">' + c.value + "</div>");
							if (!Z) {
								S = (s ? "&nbsp;" : c.value)
							}
							var K = "";
							var N = (s ? "row_null" : "row");
							var v = 0;
							if (!U && (typeof g == "undefined" || g.value === "null")) {
								v = 1;
								var u = c.properties.dimension;
								var e = c.properties.level;
								var E = (u in l ? l[u].length - l[u].indexOf(e) : 1);
								for ( var X = h + 1; v < E && X <= (a + 1) && ad[r][X] !== "null"; X++) {
									v = X - h
								}
								h = h + v - 1
							}
							y += '<th class="' + N + '" ' + (v > 0 ? ' colspan="' + v + '"' : "") + K + ">" + S
									+ "</th>"
						} else {
							if (c.type === "ROW_HEADER_HEADER") {
								y += '<th class="row_header">' + (Z ? "<div>" + c.value + "</div>" : c.value) + "</th>";
								U = true;
								R = true;
								a = h;
								if (c.properties.hasOwnProperty("dimension")) {
									var u = c.properties.dimension;
									if (!(u in l)) {
										l[u] = []
									}
									l[u].push(c.properties.level)
								}
							} else {
								if (c.type === "DATA_CELL") {
									A = true;
									...
									var G = c.value;
									var J = "";
									if (c.properties.hasOwnProperty("image")) {
										var I = c.properties.hasOwnProperty("image_height") ? " height='"
												+ c.properties.image_height + "'" : "";
										var F = c.properties.hasOwnProperty("image_width") ? " width='"
												+ c.properties.image_width + "'" : "";
										G = "<img " + I + " " + F + " style='padding-left: 5px' src='"
												+ c.properties.image + "' border='0'>"
									}
									if (c.properties.hasOwnProperty("style")) {
										H = " style='background-color: " + c.properties.style + "' "
									}
									if (c.properties.hasOwnProperty("link")) {
										G = "<a target='__blank' href='" + c.properties.link + "'>" + G + "</a>"
									}
									if (c.properties.hasOwnProperty("arrow")) {
										J = "<img height='10' width='10' style='padding-left: 5px' src='./images/arrow-"
												+ c.properties.arrow + ".gif' border='0'>"
									}
									y += '<td class="data" '
											+ H
											+ ">"
											+ (Z ? '<div class="datadiv" alt="' + c.properties.raw + '" rel="'
													+ c.properties.position + '">' : "") + G + J + (Z ? "</div>" : "")
											+ "</td>";
									if (n[ROWS]) {
										y += genTotalDataCells(W + 1, V, ac[ROWS], P[ROWS], n, Z)
									}
								}
							}
						}
		...
		var w = "";
		if (n[COLUMNS] && V >= 0) {
			w += genTotalHeaderRowCells(V + 1, ac, P, n, Z)
		}
		if (A && q) {
			if (r <= q) {
				if (!T && !Y) {
	...
	if (D) {
		D.batchResult = Q;
		D.hasBatchResult = Q.length > 0
	}
	return "<table>" + p + "</tbody></table>"
};
var SaikuChartRenderer = function(d, b) {
	this.rawdata = d;
	this.cccOptions = {};
	...
	if (b.zoom) {
		...
		$(this.el).find(".zoomout").on("click", function(e) {
			e.preventDefault();
			a.zoomout()
		});
		$(this.el).find(".zoomin").on("click", function(e) {
			e.preventDefault();
			a.zoomin()
		});
		$(this.el).find(".rerender").on("click", function(e) {
			e.preventDefault();
			$(a.el).find(".zoomout").hide();
			a.switch_chart(a.type)
		})
	}
	if (b.chartDefinition) {
	...
	if (b.adjustSizeTo) {
		this.adjustSizeTo = b.adjustSizeTo
	} else {
		this.adjustSizeTo = b.htmlObject
	}
	if (this.rawdata) {
...
SaikuChartRenderer.prototype.adjust = function() {
	var a = this;
	var c = function() {
		if (a.hasRendered && $(a.el).is(":visible")) {
			a.switch_chart(a.type)
		}
	};
...
SaikuChartRenderer.prototype.zoomin = function() {
	...
	if (e == null || e.length == 0) {
		...
		if (e.length == 1) {
			...
			pvc.data.Data.setVisible(d.datums(null, {
				visible : false
			}), true)
		} else {
			if (e.length > 1) {
				c.keptVisibleDatumSet.splice(e.length - 1, 1);
				var a = d.datums(null, {
					visible : false
				}).array();
				var b = c.keptVisibleDatumSet[e.length - 1];
				_.intersection(b, a).forEach(function(f) {
					f.setVisible(true)
				})
			}
		}
...
SaikuChartRenderer.prototype.switch_chart = function(a) {
	var b = {
		stackedBar : {
			type : "BarChart",
			stacked : true
	...
	if (a == "suanburst") {
		...
		if (b.hasOwnProperty(a)) {
			...
			alert("Do not support chart type: '" + a + "'")
		}
	}
};
SaikuChartRenderer.prototype.sunburst = function() {
	this.type = "sunburst";
	var d = this.process_data_tree( {
		data : this.rawdata
	});
	var i = this.getQuickOptions( {});
	function g(j) {
		return j.parentNode ? (g(j.parentNode) + "." + j.nodeName) : j.nodeName
	}
	...
	var c = pv.colors(i.colors).by(function(j) {
		return j.parentNode && j.parentNode.nodeName
	});
	var b = new pv.Panel().width(i.width).height(i.height).canvas(i.canvas);
	var e = b.add(pv.Layout.Partition.Fill).nodes(a).size(function(j) {
		return j.nodeValue
	}).order("descending").orient("radial");
	e.node.add(pv.Wedge).fillStyle(pv.colors(i.colors).by(function(j) {
		return j.parentNode && j.parentNode.nodeName
	})).visible(function(j) {
		return j.depth > 0
	}).strokeStyle("#000").lineWidth(0.5).text(function(k) {
		var j = "";
		if (typeof k.nodeValue != "undefined") {
			j = " : " + k.nodeValue
		}
		return (k.nodeName + j)
	}).cursor("pointer").events("all").event("mousemove", pv.Behavior.tipsy(f));
	e.label.add(pv.Label).visible(function(j) {
		return j.angle * j.outerRadius >= 6
	});
	this.chart = b
};
...
SaikuChartRenderer.prototype.getQuickOptions = function(b) {
	var d = (b && b.type) || "BarChart";
	var a = _.extend( {
		type : d,
		canvas : "canvas_" + this.id
	}, this.cccOptionsDefault.Base, this.cccOptionsDefault[d], b);
	if (this.adjustSizeTo) {
		var e = $(this.adjustSizeTo);
		if (e && e.length > 0) {
			var c = e.width() - 40;
			var f = e.height() - 40;
			if (c > 0) {
	...
	if (this.data != null && this.data.resultset.length > 5) {
		if (a.type === "HeatGridChart") {
		} else {
			if (a.orientation !== "horizontal") {
				a.extensionPoints = _.extend(Object.create(a.extensionPoints || {}), {
...
SaikuChartRenderer.prototype.define_chart = function(g) {
	if (!this.hasProcessed) {
		this.process_data_tree( {
			data : this.rawdata
		}, true, true)
	...
	var l = this;
	var h = (this.adjustSizeTo ? $(this.adjustSizeTo) : $(this.el));
	var f = (this.data != null && this.data.height < 80 && this.data.width < 80);
	var j = (this.data != null && this.data.height < 300 && this.data.width < 300);
	var e = (!f && !j);
	var c = false;
	var b = f;
	var k = h.width() - 40;
	var i = h.height() - 40;
	var d = _.clone(this.cccOptions);
	if (g && g.width) {
		k = g.width
	}
	if (g && g.height) {
	...
	if (e) {
		if (d.hasOwnProperty("extensionPoints") && d.extensionPoints.hasOwnProperty("line_interpolate")) {
			delete d.extensionPoints.line_interpolate
		}
		if (d.hasOwnProperty("extensionPoints") && d.extensionPoints.hasOwnProperty("area_interpolate")) {
			delete d.extensionPoints.area_interpolate
		}
	}
	var a = {
		legend : {
			scenes : {
				item : {
					execute : function() {
						var o = this.chart();
						if (!o.hasOwnProperty("keptVisibleDatumSet")) {
							o.keptVisibleDatumSet = []
						}
						var n = o.keptVisibleDatumSet.length > 0 ? o.keptVisibleDatumSet[o.keptVisibleDatumSet.length - 1]
								: [];
						var m = n.length > 0;
						if (m) {
							_.intersection(this.datums().array(), n).forEach(function(p) {
								p.toggleVisible()
							})
						} else {
		...
		userSelectionAction : function(n) {
			if (n.length == 0) {
				return []
			}
			var q = l.chart.root;
			var p = q.data;
			var u = this.chart;
			if (!u.hasOwnProperty("keptVisibleDatumSet")) {
				u.keptVisibleDatumSet = []
			}
			if (p.datums().count() > 1500) {
				pvc.data.Data.setSelected(n, true)
			} else {
				if (p.datums(null, {
					visible : true
				}).count() == p.datums().count()) {
					$(l.el).find(".zoomout, .rerender").show();
					var s = p.datums().array();
					_.each(_.difference(s, n), function(v) {
						v.setVisible(false)
					});
					u.keptVisibleDatumSet = [];
					u.keptVisibleDatumSet.push(n)
				} else {
					var m = u.keptVisibleDatumSet.length > 0 ? u.keptVisibleDatumSet[u.keptVisibleDatumSet.length - 1]
							: [];
					var t = p.datums(null, {
						visible : true
					}).array();
					...
					if (t.length < m.length) {
						o = t;
						u.keptVisibleDatumSet.push(t)
					}
					var r = [];
					_.each(_.difference(t, n), function(v) {
						v.setVisible(false)
					});
					_.each(_.intersection(t, n), function(v) {
						r.push(v)
					});
					if (r.length > 0) {
						u.keptVisibleDatumSet.push(r)
					}
				}
			}
			q.render(true, true, false);
			return []
		}
	};
	d = _.extend(d, {
	...
	if (d.type == "TreemapChart") {
		d.legend.scenes.item.labelText = function() {
			var m = "";
			var n = this.group;
			if (n) {
				var o = n.depth;
				...
				case 2:
					...
					break;
				default:
					m = new Array(2 * (o - 2) + 1).join(" ") + " └ "
				}
			}
			return m + this.base()
		}
	}
	this.chart = new pvc[d.type](d);
	this.chart.setData(this.data, {
		crosstabMode : true,
		seriesInRows : false
	})
};
SaikuChartRenderer.prototype.render_chart_element = function(c) {
	var b = c || this;
	var f = (b.data != null && b.data.height < 80 && b.data.width < 80);
	var h = (b.data != null && b.data.height < 300 && b.data.width < 300);
	var d = (!f && !h);
	var a = false;
	if (b.chart.options && b.chart.options.animate) {
		a = true
	}
	if (!a || $(b.el).find(".canvas_wrapper").is(":visible")) {
		$(b.el).find(".canvas_wrapper").hide()
	}
	try {
		if (a) {
			$(b.el).find(".canvas_wrapper").show()
		}
		b.chart.render();
		b.hasRendered = true
	} catch (g) {
		$("#canvas_" + b.id).text("Could not render chart" + g)
	}
	if (b.chart.options && b.chart.options.animate) {
		return false
	}
	if (isIE || d) {
		...
		$(b.el).find(".canvas_wrapper").fadeIn(400)
	}
	return false
};
SaikuChartRenderer.prototype.process_data_tree = function(f, s, F) {
	var x = this;
	var E = {};
	if (s) {
		E.resultset = [];
	...
	var w = E;
	if (typeof f == "undefined" || typeof f.data == "undefined") {
		return
	}
	if (f.data != null && f.data.error != null) {
		return
	}
	if (f.data == null || (f.data.cellset && f.data.cellset.length === 0)) {
		return
	}
	var z = f.data.cellset;
	if (z && z.length > 0) {
		var h = 0;
		var a = 0;
		var C = false;
		for ( var n = 0, m = z.length; a == 0 && n < m; n++) {
			for ( var c = 0, e = z[n].length; c < e; c++) {
				if (!C) {
					while (z[n][c].type == "COLUMN_HEADER" && z[n][c].value == "null") {
						n++
					}
				}
				C = true;
				if (z[n][c].type == "ROW_HEADER_HEADER") {
					while (z[n][c].type == "ROW_HEADER_HEADER") {
						if (s) {
							E.metadata.push( {
								colIndex : c,
								colType : "String",
		...
		for ( var n = a, m = z.length; n < m; n++) {
			if (z[n][0].value !== "") {
				...
				var k = null;
				for ( var v = 0; v <= h; v++) {
					if (z[n] && z[n][v].value === "null") {
						w = E;
						var p = 0;
						for (; p < h && z[n][p].value === "null"; p++) {
							w = w[r[p]]
						}
						if (p > v) {
							v = p
				...
				l = _.clone(r);
				for ( var i = h + 1, B = z[n].length; i < B; i++) {
					var b = z[n][i];
					var y = b.value || 0;
					var A = (y != 0);
					var u = b.properties.raw;
					if (u && u !== "null") {
						y = parseFloat(u)
					} else {
						if (typeof (b.value) !== "number" && parseFloat(b.value.replace(/[^a-zA-Z 0-9.]+/g, ""))) {
							y = parseFloat(b.value.replace(/[^a-zA-Z 0-9.]+/g, ""));
							A = false
						}
					}
					if (y > 0 && A) {
						y = b.value && b.value.indexOf("%") >= 0 ? y * 100 : y
					}
					g.push(y);
					l.push( {
						f : b.value,
		...
		if (F) {
			...
			x.data = E;
			x.hasProcessed = true;
			x.data.height = x.data.resultset.length
		}
		return E
	} else {
		$(x.el).find(".canvas_wrapper").text("No results").show()
	}
};
var Tab = Backbone.View
		.extend( {
			tagName : "li",
			events : {
				"click a" : "select",
