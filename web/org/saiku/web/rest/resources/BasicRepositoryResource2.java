...
public class BasicRepositoryResource2 implements ISaikuRepository {
	 ...
	 * 
	 * @see
	 * org.saiku.web.rest.resources.ISaikuRepository#getRepository(java.lang
	...
	public List<IRepositoryObject> getRepository(@QueryParam("path") String path, @QueryParam("type") String type) {
		...
		try {
			if (path != null && (path.startsWith("/") || path.startsWith("."))) {
				throw new IllegalArgumentException("Path cannot be null or start with \"/\" or \".\" - Illegal Path: "
						+ path);
			}
	 ...
	 * 
	 * @see
	 * org.saiku.web.rest.resources.ISaikuRepository#getResource(java.lang.String
	 * )
	 */
	@GET
	@Produces( { "text/plain" })
	...
	public Response getResource(@QueryParam("file") String file) {
		try {
			...
			// repo.getName().getRelativeName(repoFile.getName()));
			if (repoFile.exists()) {
				// InputStreamReader reader = new
				// InputStreamReader(repoFile.getContent().getInputStream());
				// 中文乱码
				InputStreamReader reader = new InputStreamReader(repoFile.getContent().getInputStream(), "UTF-8");
	 ...
	 * 
	 * @see
	 * org.saiku.web.rest.resources.ISaikuRepository#saveResource(java.lang.
	 * String, java.lang.String)
	 */
	@POST
	@Path("/resource")
		...
		try {
			...
			if (!acl.canWrite(file, username, roles)) {
				return Response.serverError().status(Status.FORBIDDEN).entity(
						"You don't have permissions to save here!").type("text/plain").build();
			}
			...
			if (!StringUtils.isNotBlank(content)) {
				...
				// OutputStreamWriter(repoFile.getContent().getOutputStream());
				// 中文乱码
				OutputStreamWriter ow = new OutputStreamWriter(repoFile.getContent().getOutputStream(), "UTF-8");
	 ...
	 * 
	 * @see
	 * org.saiku.web.rest.resources.ISaikuRepository#deleteResource(java.lang
	 * .String)
	 */
	@DELETE
	@Path("/resource")
	...
	public Response moveResource(@FormParam("source") String source, @FormParam("target") String target) {
		try {
			...
			return Response.serverError().entity(
					"Cannot move resource from " + source + " to " + target + " ( " + e.getMessage() + ")").type(
					"text/plain").build();
		}
	...
	public Response getResourcesAsZip(@QueryParam("directory") String directory, @QueryParam("files") String files) {
		try {
			...
			return Response.ok(zipDoc, MediaType.APPLICATION_OCTET_STREAM).header("content-disposition",
					"attachment; filename = " + directory + ".zip").header("content-length", zipDoc.length).build();
	...
	public Response uploadArchiveZip(@QueryParam("test") String test,
			@FormDataParam("file") InputStream uploadedInputStream,
			@FormDataParam("file") FormDataContentDisposition fileDetail, @FormDataParam("directory") String directory) {
				...
				if (acl.canRead(relativePath, username, roles)) {
					...
					if (file.getType().equals(FileType.FOLDER)) {
						repoObjects.add(new RepositoryFolderObject(filename, "#" + relativePath, relativePath, acls,
								getRepositoryObjects(file, fileType)));
					}
