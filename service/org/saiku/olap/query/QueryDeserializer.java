		...
		if (queryElement != null && queryElement.getName().equals(QUERY)) {
			...
			try {
				...
				if (qmElement != null) {
					...
					SaikuCube cube = new SaikuCube(connectionName, cubeName, qm.getCube().getName(), qm.getCube()
							.getCaption(), catalogName, schemaName);
	...
	private IQuery createMdxQuery() throws QueryParseException, SQLException {
		...
		if (queryElement != null && queryElement.getName().equals(QUERY)) {
			...
			try {
				...
				if (mdxElement != null) {
					SaikuCube cube = new SaikuCube(connectionName, cubeName, cubeName, cubeName, catalogName,
							schemaName);
		...
		if (StringUtils.isNotBlank(dimName)) {
			...
			if (inclusions != null) {
				for (int z = 0; z < inclusions.getChildren(SELECTION).size(); z++) {
					...
					if ("level".equals(type)) {
						...
						sel = dim.include(Selection.Operator.valueOf(operator), IdentifierNode.parseIdentifier(name)
								.getSegmentList());
					}
					...
					if (sel != null && contextElement != null) {
						for (int h = 0; h < contextElement.getChildren(SELECTION).size(); h++) {
							...
							if (contextDim != null) {
								Selection contextSelection = contextDim.createSelection(Selection.Operator
										.valueOf(contextoperator), IdentifierNode.parseIdentifier(contextname)
										.getSegmentList());
			...
			if (inclusions != null) {
				for (int z = 0; z < exclusions.getChildren(SELECTION).size(); z++) {
					...
					String name = selectionElement.getAttributeValue("node");
					String operator = selectionElement.getAttributeValue("operator");
					dim.exclude(Selection.Operator.valueOf(operator), IdentifierNode.parseIdentifier(name)
							.getSegmentList());
					// ADD CONTEXT ?
				}
			}
			...
			for (Database db : connection.getOlapDatabases()) {
				...
				if (cat != null) {
					for (Schema schema : cat.getSchemas()) {
						if (schema.getName().equals(schemaName) || (schema.getName().equals("") && schemaName == null)) {
